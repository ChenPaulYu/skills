...
**Key design choice**: doctor does NOT auto-refactor. Refactors are high-risk and need their own narrow scope (per `nav-refactor`'s discipline). Doctor RECOMMENDS structural moves with concrete next-step proposals; you decide whether to start a `/skills:nav-refactor` session for each.
...
| **Structural** (separate session) | giants, layer violations, dead modules, fake abstractions, suspect mega-functions | Recommend specific `/skills:nav-refactor` moves; do NOT execute now |
...
### Structural recommendations (run `/skills:nav-refactor` separately)
...
- `/skills:nav-audit` — invoked as step 2 of doctor; can also be run standalone for read-only health check
- `/skills:nav-headers` — invoked as step 4 of doctor; can also be run standalone
- `/skills:nav-map` — invoked as step 5 of doctor; can also be run standalone
- `/skills:nav-refactor` — **not invoked by doctor**; recommended by doctor's final report for structural moves; user runs separately
