...
Find `blueprints/` (commonly `docs/blueprints/`). A project that already carries the tree + the `AGENTS.md` priming block — skip to Step 2. Otherwise you're **adopting an existing repo** into the workflow, and this first run scaffolds it (there is deliberately no `shape:init`):
