...
> **One current state, one maintained render.** `plan.md` (+ `decisions.md`) is the only standing artifact — already plain, readable markdown, so there's no second file to keep in sync. A human wanting a visual view renders one **on demand via `shape-mockup`**, disposable, never stored.
...
blueprints/
  ...
  mockups/           ← committed. interactive HTML decision artifacts from shape-mockup (Pick logs + ratified samples) — plus, on request, a disposable board snapshot rendering plan.md + decisions.md for a human to browse.
...
| STATE (on-demand view) | a `shape-mockup` board snapshot | human | rendered fresh when wanted from `plan.md` (status) **and** `decisions.md` (why), click-to-reveal — never stored |

Dependencies point **downstream only**: a board snapshot, when rendered, derives from `plan.md` + `decisions.md`; each references `thoughts/`. Never the reverse. Rendering a snapshot must never become a place where new decisions are born — those belong in a thought. (One human surface: the *why* is a layer **inside** the snapshot, not a separate `decisions.html`.)
...
When a human wants a visual view of the board, `shape-mockup` generates it fresh by copying [`overview-template.html`](overview-template.html) and filling its data arrays from the current `plan.md` + `decisions.md` — **disposable, not committed as a standing file**, same disposal discipline as any other mockup artifact (see mockup's `SKILL.md`). Hard rules (in the template's top comment too):
