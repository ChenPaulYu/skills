---
name: nav-map
description: "Render or refresh a bilingual repo-level codebase map from load-bearing files and headers. Use for architecture maps, module maps, or refreshing docs/codebase-map/index.html. Onboarding chat: nav-tour."
---
...
> **Cost tier (ADR-059):** this skill declares the mechanical-tier executor role in its frontmatter — rendering the map from already-maintained headers is mechanical, high-volume work, so it runs on the cheaper model for that turn; the session model resumes on the next prompt.
