...
Operates on the **content repo** — a *separate* coordination repo located via `$RELAY_REPO`, else the current dir if it has `relay.yml`, else **ask the user** (never assume cwd; see AGENTS.md). **One project at a time** (`projects/<name>/thoughts/`) — never the whole repo at once (too big; and conformance is per-project). Writes fixes to thought files; shows a diff and is gated.
