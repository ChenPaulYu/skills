...
*offer* — never auto-call — the next step **per each finding's kind**, via the Codex interactive chooser
...
`AGENTS.md`); video capture rides whatever that slot supports, falling back to
screenshots.
