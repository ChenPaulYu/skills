---
name: relay-register
description: "Add a person to a relay roster and assign their project role. Use to register, onboard, enroll, or role-assign a teammate; writes relay.yml/project.yml behind a diff gate."
---
...
Operates on the **content repo** — a *separate* coordination repo located via `$RELAY_REPO`, else a cached prior resolution, else the current dir if it has `relay.yml`, else **ask the user** and cache the answer (never assume cwd; see AGENTS.md). Writes two layers; shows a diff and is gated.
