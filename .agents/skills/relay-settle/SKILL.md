...
Operates on the **content repo** — a *separate* coordination repo located via `$RELAY_REPO`, else the current dir if it has `relay.yml`, else **ask the user** (never assume cwd; see AGENTS.md) — one project. Writes the **`index.md` snapshot** + moves settled thoughts to `archive/`; shows a diff and is gated. By convention the project **owner** runs it (one writer keeps the archive moves conflict-free); non-critical and re-runnable.
