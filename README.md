...
| [`relay`](plugins/relay/) | **Coordinate with a counterpart through GitHub** — `launch` audits repository readiness; `report` routes intent into Discussions, Issues, or pull requests; `digest` shows real obligations plus a separate non-binding notices tier; `reply` records the native response; `brief` preserves reusable cited understanding; `settle` closes with authority or makes approved Core effective. GitHub owns state; Relay owns semantics and verification. Independent. |
...
- `/relay:report` — route awareness into a receipt-default Announcement (every named recipient owes its own 👀 unless the Discussion is labeled `fyi`), evidenced work into an assigned Issue, and exact material into a requested-reviewer pull request; wanting response content rather than a bare receipt routes to Q&A instead; entry-owner discipline means an ask expecting a response must land in one of those native shapes or it's FYI; split independently completable asks and verify responsibility
- `/relay:digest` — show real GitHub obligations: per-recipient awareness receipts on Announcements (plus a close-nudge for the author once everyone has reacted), assigned work, native Q&A obligations, current-revision review rounds, author action after Request changes, a request-reviewer obligation for your own unreviewed non-draft PR, authorized PR merge, and lifecycle blockers — plus a separate, non-binding notices tier (wild traffic only — comment-borne mentions and objects with no receipt semantics) that is never presented as an obligation; an open non-draft PR is never obligation-free; read-only
