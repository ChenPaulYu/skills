...
The first thing to know before touching any skill. A skill has three layers, and **each fact lives in the lowest layer that still fires it**:

| Layer | Loaded | Carries |
|---|---|---|
| `description` (frontmatter) | **every turn**, every session | when-to-summon (the user's measured vernacular) + one NOT-boundary. ≤3 sentences. |
...
- **★ Dispatch tiers** — a second dimension on top of the cost tier above: not a skill's own declared tier, but who gets dispatched to do a piece of work. The judgment seat (planning, review/check, unblocking a stuck call) stays on the strong reasoner end to end and is never delegated to a sub-agent; reconnaissance and disciplined execution go to the cheap hand — downgrading is safe exactly when the judgment seat itself reviews the output downstream. A seat is named by **role**, never pinned to a tool or model name, so swapping either changes nothing but this line. Tier default: **judgment = the session model; reconnaissance/execution sub-agents default to `model: sonnet`** — change dispatch by editing this one line. Consultant seat, when a judgment call is stuck below confidence: **ask the user directly** — no sub-agent rung stands between a stuck call and the person who can resolve it ([ADR-122](docs/adr/122-consultant-seat-drops-the-subagent-rung.md) dropped the sub-agent-reviewer rung ADR-087/089 had built and patched; a second model instance reviewing the first's judgment just reproduces the same un-reviewed-judgment problem one level up). Reconnaissance sub-agents report back grounded in fact (file:line / source URL) — facts, not impressions. Dispatch is made **visible**, not silent — but the gate is **cost-scaled, not universal (ADR-114)**. **Cheap → run it, report after**: read-only reconnaissance, ≤2 agents, no browser/computer-use, no live-LLM fan-out. **Expensive → one real `AskUserQuestion` for the whole batch first** (ADR-040's pattern — never per-agent), listing the work items, the target tier (**the cheap hand pre-selected — that is the standing default**), and what stays with the judgment seat: expensive means **any agent that writes files**, or **≥3 agents**, or **browser / computer-use driving**, or **a live-LLM fan-out**. Why scaled: gating every dispatch equally is how a gate becomes noise and stops being read — the 2026-07-16 standing rule ("don't ask, just dispatch") exists because per-dispatch asking was worse. Cost is **measured, not estimated** — today's real token spend is computable from the local session logs, so "expensive" is a number, not a feeling. Any turn that dispatched carries a fixed return line, `⚙ 派工:執行=<tier> ×N|判斷+驗收=session model` (no dispatch = no line), and its commit message carries "Executed by a `<tier>` hand, judgment-seat reviewed (ADR-067)" — this line must live in the dispatcher's own **walked** reporting step, not a footer (a reference-doc mention doesn't fire — [`docs/observations/2026-07-14-knowledge-is-not-cue-walked-steps-fire.md`](docs/observations/2026-07-14-knowledge-is-not-cue-walked-steps-fire.md)). The user's own tier designation always beats this default, scoped to their wording (this task / this line / today); a **persistent** override is a written CLAUDE.md rule, never a remembered conversation fact. **Which task types** are safe to hand down — the three criteria (ground truth · cheap downstream verification · cheap mistakes), the ratified nine-type list, the never-downgrade list, and the brief-level fuses every cheap dispatch owes (primary-source verification · facts-not-verdicts · blocked-never-guess · no self-granted exemptions · external verification for generative probes) — are ratified in [ADR-088](docs/adr/088-task-tier-taxonomy.md). Full rationale: [ADR-067](docs/adr/067-dispatch-tiers-consultant-seat.md) · [ADR-081](docs/adr/081-dispatch-proposal-gate-and-self-report.md) · [ADR-122](docs/adr/122-consultant-seat-drops-the-subagent-rung.md).
