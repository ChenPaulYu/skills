...
- **★ Self-contained skill, three layers (ADR-109)** — a skill depends on no CLAUDE.md being loaded, but **self-contained means within the skill's directory, not within the body**. Three layers, each fact in the lowest layer that still fires it: `description` (paid every turn) = when-to-summon + one NOT-boundary, ≤3 sentences; `SKILL.md` body (paid on fire) = stance + behavior-changing gates, **delete-test each paragraph** (model behaves the same without it while running the skill → sink or cut); `references/` (paid on demand) = protocol detail, tables, rule restatements, moved **verbatim** (rule ⑥). Skill depth = behavior delta ÷ always-resident tokens; the depth test (run the prompt with and without the skill; diff ≈ 0 → don't build it) applies at authoring time, not just retirement. Gates stay in bodies — compression pressure falls on narration, never on gates.
