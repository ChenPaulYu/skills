...
> - **youtube-toolkit**(`rytho-ai/packages/youtube-toolkit` — published-grade but **flat layout + legacy black/flake8/mypy lint**)— a *contrast*, not a model: shows what NOT to copy for a rytho-ai-internal lib(see Divergences).
>
...
- **Layout = src layout.** Both rytho-ai-native libs(tactus, agent-daw)use `src/<pkg>`; only the externally-originated youtube-toolkit is flat. src also dodges the classic pytest "imports the repo dir, not the installed package" trap — no `pythonpath` hack needed.
- **Lint = ruff, line-length 100**(per stack-principles). The exemplars genuinely diverge — agent-daw configured *no* linter, youtube-toolkit still runs black+flake8+mypy@88. That's legacy/unset, **not a counter-vote**; new python-libs use ruff. ruff must be a **declared dev dep**, not just a `[tool.ruff]` block(see Gotcha 1).
...
3. **flat layout → pytest import ambiguity.** If you ever deviate to flat layout(like youtube-toolkit), pytest may import the source tree instead of the installed package and needs `[tool.pytest.ini_options] pythonpath`/conftest gymnastics. src layout avoids the whole class of problem — prefer it.
...
No server curl, no browser leg. If the lib grows a CLI face, add a 6th leg: `uv run <cli> --help` smoke.
