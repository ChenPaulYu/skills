...
5. **Install the workflow-priming layer** so the project is *born* smooth — the structure skip #5 above. Three parts (a project missing any one feels un-smooth for shape/nav; see [`../align/references/dev-workflow-stub.md`](plugins/shape/skills/align/references/dev-workflow-stub.md)):
   ...
   b. **The `blueprints/` tree** — materialize from align's templates ([`../align/references/blueprints-spec.md`](plugins/shape/skills/align/references/blueprints-spec.md) + `overview-template.html`): seed `plan.md` with the verified baseline as the first ✅ Shipped item, plus `thoughts/` · `plans/` · `mockups/`. **Weight-adaptive + consumption-gated**: seed `plan.md` always (the maintained board); scaffold a standing `overview.html` only when a human will actually read it — a multi-reader product. A solo / single-owner project (even a product) and any pure library stay lean: `plan.md` is the board, the human view is generated **on-demand** from it (see blueprints-spec's consumption gate). Don't pre-scaffold an unread render at birth — a brand-new project has zero readers yet, so default lean and add the board if a reader appears.
