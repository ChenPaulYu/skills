...
- **For a handfeel / gesture decision, the artifact must implement the *real interaction logic*, not just toggle the look.** When the decision is *how a gesture feels* (drag-to-place, edge-resize, an undo stack, snap), a static candidate with live toggles can't be judged — the user has to *do* it. So build the actual behaviour (a working throwaway prototype: real drag math, a real history stack, real snapping), and **verify it works via the browser-verify slot before handing over** — a handfeel mockup that doesn't actually respond is undecidable. (Disposable still holds: throwaway code, real behaviour.)
