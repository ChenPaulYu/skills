...
| CURSOR (ephemeral, session-to-session) | `blueprints/baton.md` (no tree → root `HANDOFF.md`) | agent, next session | where/why the last session stopped — held until read and cleared, shared by shape-park and shape-catchup |

Dependencies point **downstream only**: a board snapshot, when rendered, derives from `plan.md` + `thoughts/`. Never the reverse. Rendering a snapshot must never become a place where new decisions are born — those belong in a thought. The CURSOR row sits **outside** this chain — `baton.md` is written by shape-park and read by shape-catchup, never derived from DECISION/STATUS/VIEW and never a source for them either.
