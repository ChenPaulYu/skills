...
**Who writes it:** `setup` writes it at project birth (into the new `CLAUDE.md`); `align` ensures it on first run when adopting an *existing* repo into the workflow. Both materialize from THIS file — one source, two entry points, so they can't drift.
...
3. **A communication directive** — language + style. Skill SKILL.md files carry their own `Communication Style`, but that only loads *when a skill fires*; between skill invocations the agent drifts to its default. The project's always-loaded `CLAUDE.md` is the only surface that holds the line every turn.
