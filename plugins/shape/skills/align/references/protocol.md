...
> **Cost tier (ADR-059):** this skill declares `model: sonnet` in its frontmatter — the bulk of
...
See [`blueprints-spec.md`](plugins/shape/skills/align/references/blueprints-spec.md) for the full convention. In short:
...
- **Also install the priming layer the project lacks** (the reason adoption felt un-smooth):
  ensure the `## Dev workflow` block from [`dev-workflow-stub.md`](plugins/shape/skills/align/references/dev-workflow-stub.md) exists
