...
- **Report the tree's convention dialect — one line, every run.** You are already reading the whole tree, so the check is free: does it speak the current version (`thoughts/*.md` carrying `Status:` lines) or a legacy one (`precedents/index.md`, `decisions.md`, a `core/` tier)? Legacy → say so plainly and name `/shape:migrate` as the verb that upgrades it. **Report only — never migrate mid-pass**: a structural upgrade is a separate, separately-confirmed operation, and burying it inside a compaction would be exactly the batched destructive move both skills forbid. This line exists because a tree cannot ask to be migrated; measured 2026-08-13, five real project trees sat on a convention retired that same afternoon with nothing anywhere reporting it.
- **Clear a rotten handoff note.** If `blueprints/baton.md` (or, on a no-tree project, the root `HANDOFF.md` fallback) has a recorded SHA that no longer matches `HEAD` and content this pass has verified as shipped or absorbed, delete it in the same sweep — a stale cursor is a record that lies, and align exists to stop the record lying. (shape-catchup clears only a verified consumed note; align covers the note nobody came back for.) Never delete one whose why hasn't been absorbed somewhere durable first.
...
- **shape-park / shape-catchup** — the *ephemeral* counterpart: a "stepping away right now" session cursor is `blueprints/baton.md`, not a blueprints entry align maintains. align owns **durable** status.
