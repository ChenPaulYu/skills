---
name: build
description: "Drive the plan's In-progress column to done — autonomously, serial by default, stopping to ask the moment scope/boundary/intent drops below 90%. For each item it grounds a code plan via /nav:plan, implements with /nav:refactor discipline + the inject↔check hand-off, verifies (a browser screenshot against the item's mockup for visual items; a test-gate for behavioral ones), moves it to Shipped, and regenerates the board via /shape:align. With several chunky, fully-decided, disjoint items it may PROPOSE a parallel schedule — conflicting serial prefix + write-only parallel tail in the shared tree, ONE authoritative gate at join, check brackets serial; the user's nod starts the batch (execution parallelizes, adjudication never does — ADR-040). Fires when the user asks to \"build the In-progress\", \"implement the plan\", \"do the next items\", \"run the plan to done\", \"ship the current batch\", \"just make it\" against a blueprints/ plan. Also invokable as /shape:build. A meta-skill: it orchestrates sibling protocols (reuse-via-transcript), never re-implements them, never calls another skill directly. Language- and framework-agnostic; visual verification uses a per-project browser-verify slot (default: agent-browser)."
---
