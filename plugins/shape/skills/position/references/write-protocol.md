...
`docs/core/` (position's) and the blueprints precedents tier (reconcile's — `blueprints/precedents/`, legacy `decisions.md`; ADR-105) are both durable; route by the litmus: **overturnable by experiment / approach / bet / feature-why → precedents** (reconcile graduates it itself); **settled axiom / principle (changing it makes the thing a different product) → core** (reconcile *recommends* `/shape:position` graduation — a reminder, never a write; same hand-off pattern as its decision-change → `/shape:elicit`). Without this, a canon-grade thought's best sweep outcome is demotion into the precedents tier.
