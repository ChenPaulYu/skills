...
- **Escape hatches, checked on the user's words every turn** (never on your own sense of momentum):
  ...
  - **The user can't answer without knowledge they don't have yet** — "我打不出來" · "我沒看過" · "這我不熟" · or their answers turn into visible guesses → **stop asking and hand them something to read.** Name the *one* thing missing (not a syllabus), find **2–3 real primary sources** — verify each exists and is the actual source, never recite a plausible-looking reading list from memory, since a fabricated citation costs more than the gap did — say in one line why each matters to *this* fork, then **park the volley explicitly**: write what has converged so far (a `Status: in force`-shaped partial thought, or the baton) so resuming doesn't start from zero. Do **not** summarise the sources and carry on — the point is that he reads them first-hand; agent-chewed knowledge doesn't hold up when the next fork leans on it. Distinct from the survey leg: survey maps terrain *you* can supply from the repo and domain; this fires when the missing thing lives **outside** and he wants it at first hand.
