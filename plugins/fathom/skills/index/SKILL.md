---
name: index
description: "Index an unfamiliar repository and judge how much to trust it: anchor it at a commit (with its distance from the latest release), judge how deep the comprehension ladder needs to be, land a growing file:line anchor index, and deliver a measured trust verdict — how much to believe what you read here. Fires on 'index this repo', '幫我掃一下這個 repo', 'is this codebase trustworthy?', 'how healthy is this project?'. Read-only toward the learner: it grounds, it does not teach — /fathom:guide is the teaching door."
---
...
- **Borrow `/nav:audit`'s protocol for the sweep, not for the verdict.** Its deep-sweep mode
  (one sub-agent per domain) is the right reconnaissance shape for a large or legacy repository,
