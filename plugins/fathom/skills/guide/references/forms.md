...
| Taxonomy | **Taxonomy map** | Root = the promise, branches = families, leaves = one-line jobs; lead families visually weighted. Draw the *grouping*, not a literal tree (the tree image belongs to the ladder — see `ladder.md`): root as a slim top bar (a root floating beside a tall column leaks dead space), leaves in a strict N-column grid (free-wrapping leaves produce orphan full-width cards). |
