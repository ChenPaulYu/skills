---
name: summarize
description: "Produce a complete, objective recap of what THIS session did — what was attempted, done, decided, and changed (files / commits), in order, neutrally and exhaustively (not a selective TL;DR). Read-only; summoned; the raw input /manage:observe distills. Fires on \"summarize this session\", \"what did we do\", \"recap everything\", \"full summary of this session\", \"write up what happened\", \"give me the rundown\". Distinct from /manage:catchup (where the work stands NOW plus next — summarize is the complete log of what HAPPENED) and from /manage:observe (which selectively distills the one durable learning and writes it; summarize is the objective whole that feeds it). Also invokable as /manage:summarize."
---

# summarize — a complete, objective recap of the session

Give a **complete and objective** account of what this session *did* — the full record, in order, neutral in tone. This is the **recap** move of the meta-lane: not "where are we" (`/manage:catchup`) and not "the one keeper insight" (`/manage:observe`), but **everything that happened**, faithfully. It is often the raw input you then hand to `/manage:observe` to distill.
...
End by noting that the natural next step is **`/manage:observe`** — distill the one durable learning worth keeping from this recap into the knowledge base. Offer it; don't run it. (Summarize is objective + complete; observe is selective + durable — the recap feeds the keeper.)

## Companion skills

- **`/manage:observe`** — distills the one durable, reusable learning from the session and writes it (summarize is the complete objective whole that feeds it).
- **`/manage:catchup`** — where the work stands *now* + the next step (summarize is what *happened*; catchup is where you *are*).
