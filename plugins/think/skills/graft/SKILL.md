---
name: graft
description: "Design a novel system that rhymes with a mature one by GRAFTING it: pick a donor model whose structure resonates, map EVERY primitive of the donor onto your problem, then read each mapping as fit (adopt unchanged) / break (no image — design fresh) / adapt (borrow the structure but reshape it for your domain). The payload is the ADAPT list — where the borrowed structure had to change is where your design's identity lives; only fits = reskinning, not grafting. Forces structure: target · donor + why-it-rhymes · full primitive map · fit/break/adapt reading · adapted result. Fires on \"reason this through git/X logic\", \"what would X look like as a Y\", \"map this onto <mature model>\", \"is our design just <known system> renamed\". The disciplined middle between first-principles (invent from axioms) and lazy analogy (copy unchanged) — nearly fp's inverse. Analysis only; in-chat, offers to route (/shape:elicit, /shape:mockup, /nav:plan). Invokable as /think:graft."
---
