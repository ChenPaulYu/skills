---
name: untangle
description: "Untangle a pile of papers or documents into the relational structure of the field — how the arguments connect: which build on which (lineage), which are parallel approaches (clusters), which contradict each other, and where the questions are still contested. The core output is that relational map; add your own claim as an optional lens and it also derives a positioning view — where your claim sits, the gap it fills, a related-work priority order. Reuses /research:dissect notes when they exist. Use when you have several sources and want to *see clearly how they relate*, not just read them one by one. Fires on \"how do these papers relate\", \"untangle this literature\", \"what's the structure of this field\", \"map the relationships across these papers\", \"see how these connect\", \"where does my claim fit\", \"build a related-work landscape\", \"find the gap across these papers\", \"what's already been done on X\". For dissecting one document's argument (one skeleton), use /research:dissect; to adversarially review one paper, use /research:critique."
---
