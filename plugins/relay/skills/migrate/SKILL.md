---
name: migrate
description: "Inventory a repository's pre-model coordination state — file-based ledgers, overloaded ACK-style Discussions, commit-only decisions, ad-hoc rosters — and migrate it into the Accord memory model (decisions/briefs/core), preserving immutable provenance. Audit and mapping are mandatory before mutation; execution is idempotent and write-gated."
disable-model-invocation: true
---

# migrate — bring pre-model coordination state into the Accord memory model

This is exceptional, destructive-adjacent compatibility work, not a daily Relay verb. `migrate` inventories whatever a repository used to coordinate before it adopted the memory model this plugin implements (the blueprint; `plugins/relay/CLAUDE.md`), and promotes only what is still reusable knowledge or a live obligation. Immutable git/GitHub history is the archive; migration never copies everything forward merely to preserve it.
...
- Never create `legacy/`, `raw/`, `archive/`, or time-bucket directories — the same law as ordinary daily work (`plugins/relay/CLAUDE.md`, "Raw, briefs, and Core").
