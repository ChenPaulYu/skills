...
- Read recent `thoughts/`, **stitch threads by the `re:` backlink** (a thought ← the reviews whose `re:` points at it), and read each thought's `subject`. This `re:` graph is how "settled" is **computed**: an ask with an answering `agree` on its thread is settled — there is no stored status field.
