...
- **conclusion body** — a **tone, not a `kind`**. When a discussion is too long / iterated for a one-line `log.md` entry, write a thought that **synthesizes** it (the outcome + why, referencing the `thread`); `settle` then points `log.md` at this thought instead of inlining a line. Write one **only when the thread doesn't read linearly into the conclusion** — never to restate the thread.
