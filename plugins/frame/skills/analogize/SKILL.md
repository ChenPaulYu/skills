...
- **Output proportional to the question.** Usually lead with one plain sentence, then the analogy, then only the technical detail needed for the user to use it. Name where the analogy breaks when that risk matters; for tiny explanations, one clear caveat is enough. Technical words are allowed when they help, but explain the first important one briefly.
