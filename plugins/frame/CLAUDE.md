...
- **Reasoning lenses (down/sideways/trial)** — apply a frame to a **problem**, for the agent's own understanding: strip it to axioms, factor it into independent axes, or put a claim on trial.
...
Same discipline (test the reasoning, not a prescribed display), opposite direction of transfer: the lenses derive insight; analogize delivers it. The checks earn the skill, not the number of headings.
...
Five members today — three reasoning lenses and two delivery skills:
...
- `draw` — choose a useful visual form and medium for something already understood; no fixed presentation format.

Future members are added **by evidence** (ADR-018), not pre-listed — each must clear the value-guardrail (a substantive reasoning check the default skips) AND show real recurring use. (The originally-speculated `invert` / `second-order` seed was dropped for having no usage evidence — see `docs/adr/046-think-orthogonal-lens-drop-speculative-seed.md`; charter in `docs/adr/034-think-plugin.md`.) The same bar cuts the other way too: `graft` — borrow-a-mature-model, once the fourth lens — was retired as the weakest member of the set (1 inbound reference, never invoked in 11 months of transcripts); see [ADR-107](docs/adr/107-retirement-round-four-verbs.md). `analogize` itself is provisional in this same sense — its trigger phrasing below is a first cut, expected to sharpen once real use shows which phrasings actually preceded wanting it.

This plugin lives inside the `skills` marketplace (`ChenPaulYu/skills`). It is **independent** — no dependency on `nav` or `shape`. It **feeds shape one-way** (a reasoning artifact → an `elicit` thought / a `mockup` / a `nav:plan`), a guarded, one-way pattern; it never invokes another plugin's skill (ADR-015). `analogize` is the one member that does **not** feed shape — an explanation has nothing further to converge or build (see its own SKILL.md).

## The frame through-line

The unit is a named frame applied explicitly. Each skill preserves its substantive reasoning checks while scaling presentation to the question (ADR-128).
...
1. **A named operation** — preserve the checks that distinguish the frame; use established context rather than restart a completed derivation.
2. **A reasoning difference, not a worksheet** — first-principles separates facts from inherited assumptions; orthogonal tests independence; dialectic strengthens both sides and distinguishes contradiction from missing evidence; analogize tests structural fit and limitations. Tables, headings, fixed order, and candidate counts are not evidence of quality.
...
4. **Scope-aware continuation** — analysis-only requests end with the finding. A larger explicitly authorized workflow may continue after analysis, using its execution checks. No compulsory route menu, new authority, or automatic skill invocation follows from a finding.
...
- **Naming — two idioms, side by side.** The three reasoning lenses use the **canonical lens name** — `first-principles`, `orthogonal`, `dialectic` — not a coerced bare verb; the names are well-known reasoning concepts and discoverability beats verb-purity here (the documented divergence from the marketplace bare-verb default, ADR-027). `analogize` uses the marketplace's default **bare verb** instead — it names an action (build-and-deliver-an-analogy), not a citable concept the way "first principles" or "dialectic" are. Both idioms coexist deliberately; don't force one onto the other.
- **Proportional output** — a small question can receive a short answer with the material reasoning and uncertainty visible; use fuller structure only when it helps inspection. Worked examples are illustrations, not required forms.
...
- **Optional continuation** — suggest a useful next step only when needed and not already settled; ask a specific question when a consequential choice or authority is missing. Do not turn an already-authorized continuation into another chooser.
- **Plain language without a performance** — use technical terms when useful, explaining unfamiliar ones briefly. Analogies are optional aids outside analogize; no banned-word list or compulsory closing metaphor (ADR-128 supersedes ADR-077's presentation gates).
...
- **New member — first check the value-guardrail**: name the reasoning check this frame adds that the default omits (whether the default is "reason about it" or "explain it simply"). If you can't, it's not a skill (the caution that retired nav's `doctor`, ADR-021).
