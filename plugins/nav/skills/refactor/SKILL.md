...
- **Header sync in the same change (Step 7).** A move changes file roles + `Reads` — update each touched file's top header now, not in a later `/nav:sync`. Same `stale header = lie` gate `/nav:do` enforces inline; only the repo-level *map* waits for `/nav:sync`'s map leg.
