---
name: compose
description: "Author or restructure a prose document so it reads as a deep module — lead with the point (head-able), one fact one owner (link, don't restate), grouped by knowledge not chronology, right grain, with a self-describing top. Use whenever the user asks to \"write / draft a doc\", \"author an ADR / design doc / spec / README / report\", \"structure this document\", \"turn these notes into a document\", \"clean up / restructure this doc\", \"make this doc clearer / tighten this prose\", or \"make this doc navigable\". Owns document-level structure AND the durable sentence keystones (lead with the point, active voice, omit needless words, be specific) — the self-sufficient prose-writing door. For file-top headers on CODE files use /nav:sync — compose is their prose-document sibling. A deep copyedit long-tail (grammar minutiae, commonly-misused-words) lives in the optional external writing-clearly-and-concisely skill. Writes/edits the document, gated by a diff."
---
...
5. **Fit the framework** — use the genre's idiom (ADR → Context/Decision/Consequences; report → its buckets; README → quick-start first). Don't fight the document type's conventions.
