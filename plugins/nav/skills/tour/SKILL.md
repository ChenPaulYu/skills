---
name: tour
description: "Guide a user through what the current codebase does, how its main parts cooperate, and which recorded decisions shaped it — then reconcile the agent's model with the user's corrections. Two modes: a quick two-turn reconcile (default), and a summoned deep teach-converge loop (layered teaching + reverse quizzes, iterated until the two models converge). Read-only and in-chat, for conversational codebase walkthroughs, onboarding, or shared-understanding checks; not a map refresh (/nav:sync's map leg), a work-status catchup (/reflect:catchup), or a pre-decision survey (/shape:survey)."
---
