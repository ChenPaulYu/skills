...
- **Three modes, chosen by scope + framing.** Mode 1 (no target — broad health check across all domains) · Mode 2 (a spec/plan/feature description — gap analysis scoped to the domains it touches; read-only quick check) · Mode 3 (explicit "deep audit" / "徹底掃描", or accepted from Mode 1's auto-suggest — fan-out one sub-agent per domain instead of one pass). Mode 3 is never auto-run — the fan-out costs N× a normal audit, so the spend is always the user's call. Its dispatch is read-only by construction — one message, multiple `Agent` calls; `subagent_type=Explore` — read-only is the point — each brief injected per `references/deep-sweep.md`. Reconnaissance sub-agents default to cheap tier (`model: sonnet`); workers return evidence; interpretation and acceptance stay with the session model. Resolve missing user intent with the user, never a reviewer sub-agent.
