...
Each sub-agent returns its domain's findings + self-eval. It does **not** write files (read-only audit). Reconnaissance sub-agents default to cheap tier (`model: sonnet`); workers return evidence; interpretation and acceptance stay with the session model. Resolve missing user intent with the user, never a reviewer sub-agent.
