...
> **Current as of 2026-07-22 (relay 2.0.0).** Relay is a compact semantic layer over one GitHub repository, following the Accord memory model. [ADR-090](docs/adr/090-relay-github-native.md) owns the GitHub-native strategy reversal; [ADR-100](docs/adr/100-relay-adopts-the-accord-memory-model.md) owns the memory-model adoption and points at the design record itself, [`blueprints/plans/2026-07-22-accord-memory-model.md`](blueprints/plans/2026-07-22-accord-memory-model.md); `plugins/relay/CLAUDE.md` owns the operative contract.
...
## LEGACY: `[ACK]` Discussions

An open, `[ACK]`-titled Discussion created before this model keeps its **original pre-097 single-recipient** `👀` semantics — the first `@mention` in title/body is the designated recipient, later mentions own nothing, and there is **no close-nudge** (the initiator closes on their own judgment, same as any other Discussion). This path is marked LEGACY throughout the code and skills, and is expected to retire once legacy `[ACK]` Discussions have been migrated off; new traffic should never create one. See [ADR-100](docs/adr/100-relay-adopts-the-accord-memory-model.md).
