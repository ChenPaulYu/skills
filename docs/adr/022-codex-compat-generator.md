...
4. **Synthesises `AGENTS.md`** at the repo root from the two plugin `CLAUDE.md` files (same rewrites, agents profile) — the home for the shared rules the skills reference (the 8 rules, shape's browser-verify slot).
