...
The cost of those two gaps is documented in the origin observation. A Phase-3 sub-agent, briefed only to build a small preview component, re-implemented a peaks-to-path helper function that already existed (with an important pixel-bucketing constant) in the same audio domain's renderer file. Tests passed, the parent accepted "done", and the parallel impl shipped — caught a session later **by the user, not the parent**. The observation's lever analysis scored the fix:
