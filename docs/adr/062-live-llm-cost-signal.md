...
Real incident, 2026-07-01: `crate-backend`'s dig feature had a dead fan-out path (an `Agent`/subagent tool call stripped by `--bare`) fixed same-day. The fix flipped the path from a no-op to a live one — 3–6 researcher subagents (`claude-sonnet-4.6`, up to 50 turns each) per dig call. Iterative re-testing across ~30 commits that day (each closed by a do → verify loop, hands-on real-app checks in dogfood's style) burned roughly $34 in a few hours. Root-caused by cross-referencing local Claude Code session logs against `git log` timestamps in the affected repo — the fan-out fix commit landed at the exact minute the usage burst began.
