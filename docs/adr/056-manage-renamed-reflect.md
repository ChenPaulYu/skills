...
- **A marketplace rename is NOT an install migration — each consumer must uninstall the old + install the new.** Renaming the plugin in `marketplace.json` only changes the *catalog*; a consumer's `~/.claude/plugins/installed_plugins.json` still pins the dead name (`manage@skills`). On next load that pinned-but-no-longer-cataloged entry is a dangling install → the surfaced "N error(s) during load", *and* the new `reflect` never loads because catalog-presence ≠ installed. The fix is two steps per machine, not a string edit: `claude plugin uninstall manage@skills && claude plugin install reflect@skills` (then `marketplace update skills` first if the catalog is stale, and delete the orphaned `~/.claude/plugins/cache/skills/manage/` shell). Rule of thumb: **rename = old uninstall + new install**, run wherever the plugin was installed.
