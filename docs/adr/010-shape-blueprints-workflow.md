...
- **`reconcile` includes cleanup, not just a check.** The user needs the docs *cleaned*, not only flagged. But cleanup is destructive → the skill carries explicit safety rules (verify tracked/untracked, never `mv`-then-`rm`, diff before delete, per-file confirmation), derived from the data-loss incident during the reorg that motivated this ADR.
