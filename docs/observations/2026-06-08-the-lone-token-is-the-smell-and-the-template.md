...
- The frontend's main stylesheet has **one** `@theme` token — a single motion-easing curve — and its own comment explains the point: *"the curve has a single owner instead of being copy-pasted per component."* Motion is owned.
- The **brand palette is the opposite**: a brand accent color appears as raw hex across **11 files / 27 times**; the full palette is ~10 colors, ~70 raw occurrences, ~15 files. Color is leaked.

So the honest answer wasn't "yes" or "no" — it was: *the codebase already knows this pattern (motion), it just never extended it to a sibling decision (color).* The fix wrote itself: add a `@theme` color token, migrate the hex to the generated utility class. A tracer-bullet slice proved it pixel-identical (the computed background color matched exactly the old hex).

## The signal

**An abstraction applied to one design-decision but not its siblings of the same kind is a precise, high-value leakage smell — sharper than "this is duplicated," because the codebase has already ruled on the question.** The lone motion-easing token does three jobs at once:
...
2. **Template.** The fix is not invention — it's copying the shape of the blessed instance (the motion token → the new color token; same `@theme`, same dual-use as utility + `var()`).
