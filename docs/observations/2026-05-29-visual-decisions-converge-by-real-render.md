...
- **單一來源(2026-05-29)**:一整個 Crate UI session —— reset-zoom `1:1`→`=`(commit `c24f3f0`)、卡片型別 glyph + hover chip(`ea91532`)、gutter「Track」truncate + min-width 修正(`be07b37` / `c9cae83`)。每個視覺定案都前置「render 真候選」;兩次 `AskUserQuestion` 被拒 = 直接證據;HTML mockup(`crate/mockups/2026-05-29-card-type-glyph/`,gitignored)= 收斂載具。
