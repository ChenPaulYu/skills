...
2. **All exemplars are invisible downstream.** A user who installed/forked this skill cannot clone `<internal-monorepo>/<capability-package>` — it's private. The razor tells them "ground the living exemplar"; they *have no exemplar*, only the prose the razor distrusts. For them the skill's stated method is unrunnable, and the fallback is precisely the drift-prone prose.
