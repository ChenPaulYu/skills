...
1. **spec / mockup / plan 都亂,且互相依賴** → 做到一半想改 plan 或 spec 很麻煩(要手動追相依)。證據:`crate/docs/specs/plan.md` 一份裡同時塞 spec 指標(WHAT)+ 狀態表(STATE)+ 出貨順序(HOW);spec 互相引用、還引用已刪除的 spec;mockup 只有 5/13 有 README。
