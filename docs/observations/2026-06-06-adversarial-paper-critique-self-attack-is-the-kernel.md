...
> Source: one long session writing a conference referee report for an audio-generation paper (details anonymized — reviews are confidential). The review went through ~30 rounds and landed at *Weak Accept, conditional on re-scoping*. This is the **assessment** counterpart to [[2026-06-03-paper-analysis-gap-claim-mechanism-evidence]] (which captures *dissecting* a paper — Gap/Claim/Mechanism/Evidence — i.e. understanding what it argues). Dissect = "what does it claim?"; this = "does the evidence hold, and what survives an adversary?". **Candidate skill** `research:critique` (a referee verb to sit beside `dissect` + `map`). Not promoted: `raw`, one session.

## What happened — the moves that repeated

Across every claim and every RQ, the same handful of analytical moves recurred:

1. **Pull the verbatim claim** from the abstract / contributions (e.g. "the decomposition improves adherence **and** progression without sacrificing quality").
...
3. **Ladder attribution** — when there's an ablation chain (base → intermediate → full method), locate *which step* the gain comes from. Here the big jump was at a near-tautological conditioning step borrowed from a cited prior work, while the *proposed* step actually *lowered* the headline metric.
4. **Metric scrutiny** — does the headline composite drop the axis the method loses on? (It reported a two-axis composite and omitted the third axis, on which the method fell below baselines.)
5. **Novelty bounding** — decompose the method into prior components (the key equation = two known guidance techniques ⊕ a prior work's backbone) to size the true delta.
...
- "Missing a particular baseline" → cut (not load-bearing).
- "They should position the key equation vs its components" → cut (the paper *does*).
- "The metric circularity is acute" → downgraded to a narrow caveat (it misses the circularity-free headline metric and partly cancels in the head-to-head).
...
**Tell that you skipped the self-attack:** a criticism that sounds sharp but flips the moment you recompute one number or read the claim charitably. Notably, in this session most of these were caught only because *the user* pushed ("is the intermediate variant actually their proposal?", "isn't that composite already the tradeoff?", "do they really not position the equation?"). A skill should make the self-attack a **default, proactive pass**, not something the human has to force.
