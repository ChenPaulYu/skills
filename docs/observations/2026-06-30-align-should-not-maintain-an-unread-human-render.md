...
- **Applied 2026-06-30** (maintainer ran observe → skill change directly, skipping the ADR step). The fix landed in the **single owner of the overview.html convention** (`align/references/blueprints-spec.md` § Weight-adaptive — the consumption-gate paragraph), with the consumers deferring to it: `align/SKILL.md` (spine corollary + Step 5 gate), `setup/SKILL.md` (don't pre-scaffold an unread render at birth), and `reconcile/SKILL.md` (its currency sweep + re-sync offer now RETIRE an already-out-there unread `overview.html` instead of refreshing it — the cleanup verb that reaches existing projects). Putting it only in align would have leaked the same rule the shared spec contradicted — the user caught both that ("這樣 setup 也要改?") and the existing-project cleanup gap. Stays `raw` as the single grounding case; a second project would graduate it to an ADR.
