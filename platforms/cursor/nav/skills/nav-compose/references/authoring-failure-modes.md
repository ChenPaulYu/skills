...
**Repo instance (already in use, just not named as one technique until now)**: this repo already reuses several such terms verbatim rather than re-describing them in prose each time — *"summoned, not automatic"* (`plugins/shape/skills/park/SKILL.md`, `shape-elicit`, and others, reused verbatim), *"one fact one owner"* (`the plugin conventions` cites `nav-compose`'s discipline by this phrase rather than re-defining it), *"right grain"* (the standing name for rule ④'s whole paragraph), *"stop on the snap"* (elicit's convergence-exit criterion), *"inject → check"* (the sub-agent hand-off protocol, one term standing in for the whole bracket). This document only makes explicit that these are all instances of the same rhetorical device, worth reusing deliberately rather than re-prosing each time.
