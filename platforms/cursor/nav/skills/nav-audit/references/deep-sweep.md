...
Dispatch the domains **in parallel** (one message, multiple read-only `Task` calls with `subagent_type: "generalPurpose"` — read-only is the point). Into **each** brief inject what the tactical sub-agent can't see:
...
Each sub-agent returns its domain's findings + self-eval. It does **not** write files (read-only audit). Reconnaissance sub-agents default to cheap tier (the mechanical-tier executor role); workers return evidence; interpretation and acceptance stay with the session model. Resolve missing user intent with the user, never a reviewer sub-agent.
