...
Each sub-agent returns its domain's findings + self-eval. It does **not** write files (read-only audit). Reconnaissance sub-agents default to cheap tier (the mechanical-tier executor role); a domain whose judgment call is unusually dense can be escalated on the spot (see root CLAUDE.md's Dispatch tiers).
