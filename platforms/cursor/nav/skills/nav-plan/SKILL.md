...
- **Offer next action, don't make the user type the next command.** Present an `AskQuestion` with a sub-agent default (recommended), inline execution, or save-only. Dispatching a sub-agent **injects** the grounding it can't re-derive (Critical files, existing seams to reuse, the N+1 trigger) and **checks** the returned diff (same-domain parallel impl, seam/facade intent, header hygiene, board sync) before accepting "done" — see ADR-008. Guarded, one-shot: skip when there's no executable step 1 yet, and never re-offer after a decline.
