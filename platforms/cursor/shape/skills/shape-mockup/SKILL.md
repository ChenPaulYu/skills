...
---
<!-- GENERATED by scripts/build-cursor.mjs from plugins/shape/skills/mockup/ — do not hand-edit; re-run the generator. -->
...
A pick has two natural next steps, and the offer should name **both** (ADR-028) — an `AskQuestion` with a "just record the pick, I'll continue later" opt-out (offer-next-action, ADR-007/015):
