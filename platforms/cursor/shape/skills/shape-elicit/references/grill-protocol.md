...
Then it flows the same way: converge to a one-line cause + fix-direction; if the fix is visual/structural, hand to `shape-mockup` (e.g. "right-click vs visible button"); hand the rebuild to `nav-do` or a delegated sub-agent (inject↔check). **Read-only — elicit finds the cause and converges the fix-direction; it does not edit the code in place.**
...
6. **Offer the next step (don't auto-run).** After landing the thought, *offer* — never auto-call — the natural continuation via `AskQuestion` (offer-next-action, ADR-007/015). Branches, composed from what the grill actually surfaced:
...
- That doc is now input to `shape-align` (triaged into the plan) and eventually `nav-plan` (grounded into code). In diagnostic mode it also hands off to `shape-mockup` (visual fix) and `nav-do` or a delegated sub-agent (the rebuild).
